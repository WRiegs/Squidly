...
def create_parser():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether to store the hidden layer representations of the active sites"
    )
    return parser
...
def main(args):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (labels, strs, toks) in enumerate(data_loader):
            ...
            for i, label in enumerate(labels):
                ...
                print(AS_contrastive_rep)
                
                print(AS_contrastive_rep[0].size())
                print(AS_contrastive_rep[1].size())
                print(AS_contrastive_rep[2].size())
                
                hidden_reps_1 = AS_contrastive_rep[0]
                hidden_reps_2 = AS_contrastive_rep[1]
                AS_contrastive_rep = AS_contrastive_rep[2]
                
                if args.store_hidden_reps:
                    # make a directory in the output directory to store the hidden representations
                    hidden_dir = args.output_dir / 'squidly_reps'
                    hidden_dir.mkdir(parents=True, exist_ok=True)
                    # save the hidden and output representations
                    hidden_reps_1 = hidden_reps_1.cpu()
                    hidden_reps_2 = hidden_reps_2.cpu()
                    AS_contrastive_rep = AS_contrastive_rep.cpu()
                    torch.save(hidden_reps_1, hidden_dir / f"{label}_hidden_rep1.pt")
                    torch.save(hidden_reps_2, hidden_dir / f"{label}_hidden_rep2.pt")
                    torch.save(AS_contrastive_rep, hidden_dir / f"{label}_output_rep.pt")
    ...
    if csv is not None:
        ...
        results_df.to_csv(str(args.output_dir / args.file.stem) + '_results.tsv')
        args.file.unlink()  
    else:
        results = []
        for label, AS_residues, AS_probs, AS_site_reps in zip(embeddings_labels, predicted_AS_residues, CR_probabilities_predicted, active_sites_reps):        
